...
private:
    ...
    bool find_CUBE(float surface_height, geometry_msgs::PoseStamped &object_pose);
...
bool ObjectFinder::find_CUBE(float surface_height, geometry_msgs::PoseStamped &object_pose) {
    Eigen::Vector3f plane_normal;
    double plane_dist;
    //bool valid_plane;
    Eigen::Vector3f major_axis;
    ...
    switch (object_id) {
        ...
        case ObjectIdCodes::TOY_BLOCK_ID:
            ...
            if (found_object) {
                ...
                ROS_WARN("could not find toy block");
                result_.found_object_code = object_finder::objectFinderResult::OBJECT_NOT_FOUND;
                object_finder_as_.setAborted(result_);
            }
            break;
        case ObjectIdCodes::CUBE_ID:
            //specialized function to find cube model
            found_object = find_CUBE(surface_height, object_pose); //special case for cube
            if (found_object) {
                ROS_INFO("found cube!");
                result_.found_object_code = object_finder::objectFinderResult::OBJECT_FOUND;
                result_.object_pose = object_pose;
